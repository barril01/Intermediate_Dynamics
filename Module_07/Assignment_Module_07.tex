...
$$ \bar{v}_G = \cancelto{0}{\bar{v}_p} + \bar{\omega} \times \bar{r}_{G/p} $$

This evaluates to:


$$ \bar{v}_G = \bar{\omega} \times \bar{r}_{G/p}  = \left( -\phi \bar{i}+ \Psi \bar{k}  \right) \times R \bar{k} = R \dot{\phi}\ \bar{j}  $$

The rotation from the body frame to the inertial frame de defined by the following sequence of rotations:
...
% Looking at the terms $a_{14}$ and $a_{24}$, since the terms do not contain $\phi$, there is no derivative that can result in:

% $$ \frac{\partial f_1}{\partial \phi} \ne -R\ \,\cos\left(\Psi\right) \dot{\phi} $$
% $$ \frac{\partial f_2}{\partial \phi} \ne -R\ \,\sin\left(\Psi\right) \dot{\phi} $$
